...
|---|---|---|---|---|
| Firecrawl MCP | Keep or add wording that sends one known page plus specific fields to `firecrawl_scrape`. Reserve `firecrawl_extract` for multi-page or broad extraction. | `legacy_firecrawl_mcp` chose `firecrawl_extract` for one known URL plus structured fields. | The same request routes to `firecrawl_scrape`. | `tuned_firecrawl_mcp_boundaries` moved the focused adversarial slice to 12/12. |
| Supabase MCP | Route schema-changing SQL to `apply_migration`. Use `execute_sql` only for ordinary non-schema SQL. | `terse_supabase_database_mcp` chose `execute_sql` for schema-changing SQL. | DDL and RLS changes route to `apply_migration`. | `tuned_supabase_database_boundaries` moved the full Anthropic run from 6/9 to 9/9. |
| Zymtrace MCP | Tighten default-project, resource-first, metrics-first, and bounded hot-trace routing. Add the retained cases to regression coverage. | `stock_zymtrace_mcp` missed default-project, metrics-first, resource fallback, and bounded drilldown arguments. | Agents see explicit default, resource, metrics, and bounded trace-drilldown boundaries. | `tuned_zymtrace_mcp_boundaries` moved the held-out prompt-JSON slice from 14/24 stock passes to 24/24 tuned passes. |
| Screenpipe MCP | Send literal keyword or exact phrase lookup to `keyword-search`. Reserve `search-content` for broader content search. | `readme_screenpipe_mcp` routed exact keyword lookup to `search-content`. | The same request routes to `keyword-search`. | `source_tuned_screenpipe_mcp` moved Anthropic prompt JSON from 6/7 to 7/7. |
| InsForge MCP | Require an absolute `sourceDirectory`. Reject relative deploy paths before calling `create-deployment`. | `readme_insforge_mcp` called `create-deployment` for a relative source path. | Relative deploy paths stop before tool execution. | `source_tuned_insforge_mcp` chose `NO_TOOL` and moved Anthropic prompt JSON from 15/16 to 16/16. |
| gstack skills | Tighten browser alias and safety-mode routing. Keep the current frontier failures as the next regression targets. | `gstack_stock_skill_descriptions` scored 0.975 across the original full live run. | Boundary-tuned skill descriptions clarify alias and safety-mode routing. | `gstack_boundary_tuned_skill_descriptions` scored 0.992, with current frontier receipts retained as descent evidence. |
| Humwork MCP | No wording change promoted from this slice. | `readme_humwork_mcp` already passed 7/7. | Keep current wording and retain the cases. | `skill_tuned_humwork_mcp` also passed 7/7. Guardrail only. |
| OpenWork UI MCP | No wording change promoted from this slice. | `docs_openwork_ui_mcp` already passed 7/7. | Keep current wording and retain the cases. | `source_tuned_openwork_ui_mcp` also passed 7/7. Guardrail only. |
